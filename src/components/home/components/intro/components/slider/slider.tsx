...
const Slider = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentImageIndex((prev) =>
        prev < STONE_IMAGE_ITEMS.length - 1 ? prev + 1 : 0
      );
    }, 4000);
  ...
  const renderGalleryImage = useMemo(() => {
    return STONE_IMAGE_ITEMS.map((it, index) => (
      <SAImage
        key={it.id}
        src={it.image}
  ...
  return (
    <div className="relative w-full h-full z-10 ">{renderGalleryImage}</div>
  );
};
